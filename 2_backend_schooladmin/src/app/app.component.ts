...
export class AppComponent implements OnInit, OnDestroy
{
    ...
    getSchoolID()
    {
        ...
        this._global.getSchoolFromUser(data).subscribe( (data: any) => { this._localSession.saveSchoolID(data['school_id']); },error => {});
    }
    ...
    getPurchased()
    {
        this.purchased = this._localSession.retrievePurchased();
        let user_id = this._localSession.retrieveUserID();

        this.model['token'] = this._localSession.retrieveToken();
        this.model['user_id'] = this._localSession.retrieveUserID();
        this._service.confirmPurchase(this.model, user_id).subscribe( 
            (data: any) => { 
                if ( data['status'] === 0 )
                    this.purchased = 'false';
                else if ( data['status'] === 0 && data['code'] === 200 )
                    this.purchased = 'true';

                this._localSession.savePurchased(this.purchased);
            },error => {});
    }
    /**
     * On init
